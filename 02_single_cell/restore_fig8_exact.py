"""Restore Fig8 EXACTLY from original 05_virtual_ko_final.py code."""
...
# ============================================================
# 8. Generate FIGURE — EXACT original style from 05_virtual_ko_final.py
# ============================================================
...
# ax.set_title("A. CD4 T cells: Perturbed by SESN3 KO", fontweight="bold")  # removed per user request
ax.invert_yaxis()
...
ax.set_xlabel("-log10(P.adj)")
# ax.set_title("B. CD8 T cells: Perturbed by SESN3 KO", fontweight="bold")  # removed per user request
ax.invert_yaxis()
ax.axvline(x=-np.log10(0.05), color="red", linestyle="--", alpha=0.5)
...
if len(enrich_df) > 0:
    ...
    # ax.set_title("C. Pathway Enrichment (CD4 perturbed)", fontweight="bold")  # removed per user request
    ax.invert_yaxis()
    ax.axvline(x=-np.log10(0.05), color="red", linestyle="--", alpha=0.5)
else:
...
# ax.set_title("D. Negative Control Comparison", fontweight="bold")  # removed per user request
...
# ax.set_title("E. SESN3-KO Perturbed ∩ MG DEGs", fontweight="bold")  # removed per user request
...
# ax.set_title("F. SESN3 Co-expression (CD4 T cells, HC)", fontweight="bold")  # removed per user request
ax.axvline(x=0, color="black", linewidth=0.5)

# plt.suptitle removed per user request
plt.tight_layout()

# Save to BOTH Fig8 filenames
...
print(f"\n{'='*60}")
print(f"Fig8 restored! EXACT original 05_virtual_ko_final.py code.")
